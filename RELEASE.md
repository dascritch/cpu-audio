...
* You can set some global parameters via a `<script data-cpu-audio>` tag in the `<head>` of the host page ([#185](https://github.com/dascritch/cpu-audio/issues/185))
* Can scroll the viewport of the page on the player when clicking on a temporal link. Parametrable setting ([#60](https://github.com/dascritch/cpu-audio/issues/60))
* Being able to not include global.css on host page ([#181](https://github.com/dascritch/cpu-audio/issues/181))
* Automated ID prefix for not identified `<audio>` is now parametrable ([#186](https://github.com/dascritch/cpu-audio/issues/186))
...
* Locales are modularized : each language has its own file, in `src/locales` ([#189](https://github.com/dascritch/cpu-audio/issues/189))
...
* Panels unfoldable via the use of `<details open><summary>` ([#75](https://github.com/dascritch/cpu-audio/issues/75))
* Sticky panel title ([#75](https://github.com/dascritch/cpu-audio/issues/75))
...
* Correcting `title=""` on cue points, should not put HTML tags anymore ([#178](https://github.com/dascritch/cpu-audio/issues/178))
...
* Changing license from GPL to LGPL ([#174](https://github.com/dascritch/cpu-audio/issues/174))
* Removing an example that incits a bad habit
* Correct credit for [@tdd](https://github.com/tdd)
* Regression on `<a>` tags with actions ([#170](https://github.com/dascritch/cpu-audio/issues/170)), released as 7.0.1
 ...
 * Persistent `<CPU-Controller>` for single page web-app, with current playing remanence ([#87](https://github.com/dascritch/cpu-audio/issues/87))
 * Added a [theme with a direct download button](examples/Build_with_download_action.html), instead of the action button. Example for creating graphic theme ([#56](https://github.com/dascritch/cpu-audio/issues/56))
 * Added a [theme with a simple big square button](examples/Theme_big-square.html). Example for creating leaner graphic theme ([#56](https://github.com/dascritch/cpu-audio/issues/56))
 * Possibility to create themed versions with specific html and css files ([#56](https://github.com/dascritch/cpu-audio/issues/56))
 * Possibility to change css breakpoints ([#51](https://github.com/dascritch/cpu-audio/issues/51)) by this way ([#56](https://github.com/dascritch/cpu-audio/issues/56))
 * Possibility to create RTL version ([#26](https://github.com/dascritch/cpu-audio/issues/26)) by this way ([#56](https://github.com/dascritch/cpu-audio/issues/56))
 * A11y : Support reduced motion preferences
 * Adding support for <kbd>↑</kbd> and <kbd>↓</kbd> keys, moving focus among panels
 * Adding support for `prevcue`/`nextcue` facultative buttons ([#108](https://github.com/dascritch/cpu-audio/issues/108))
 * Adding support for `prevtrack`/`nexttrack` facultative buttons ([#125](https://github.com/dascritch/cpu-audio/issues/125))
 * Added method `Element.CPU.bulkPoints()`
 * Some examples added
 * Degradable code, some elements are no more absolutely required in template ([#56](https://github.com/dascritch/cpu-audio/issues/56))
 * Revised [chapters-editors application](https://dascritch.github.io/cpu-audio/applications/chapters_editor.html), can now accept “uploaded” files [#126](https://github.com/dascritch/cpu-audio/issues/126)
 * [Wonderful example of React integration](https://github.com/scombat/react-cpu-audio) ([#120](https://github.com/dascritch/cpu-audio/issues/120))
 * [Attributes for the webcomponent can be live-changed](https://github.com/dascritch/cpu-audio/issues/128)
 * `<CPU-Controller>` has a specific CSS class ([#87](https://github.com/dascritch/cpu-audio/issues/87))
 * WebVTT tag translation in chapters editor ([#152](https://github.com/dascritch/cpu-audio/issues/152))
 * Added a [FAQ page](FAQ.md)

Corrections
-----------

 * Removing annoying interactions from Safari iOS ([#138](https://github.com/dascritch/cpu-audio/issues/138))
 * Resolving graphic glitches on Safari, SVG aren't correctly calculated on iOS ([#138](https://github.com/dascritch/cpu-audio/issues/138))
 * Production files are now more logically created in `build/` instead of `directory/`
 * `<CPU-Controller>` cannot be instanced twice
 * Methods and parameters names have been unified camelCase. Changes are incompatible with versions <7.0 ([#112](https://github.com/dascritch/cpu-audio/issues/112))
 * Title parameter in `Element.CPU.addPlane()` is now in `planePoint`
 * `Element.CPU.removeHighlightsPoints()` was too generic. Modification in parameters ([#109](https://github.com/dascritch/cpu-audio/issues/109))
 * `Element.CPU.build_chapters()` and `Element.CPU.build_playlist()` are no more public API
 * Some accessibilities issues on keyboard behavior and time line ([#116](https://github.com/dascritch/cpu-audio/issues/116))
 * Don't hide nymore the mouse cursor over the time-line ([#133](https://github.com/dascritch/cpu-audio/issues/133))
 * Reduce repaints, graphic glitches
 * An unuseful focusable was removed
 * Restauring alternative fine browsing mode on Safari ([#150](https://github.com/dascritch/cpu-audio/issues/150))
 * Do not change color when changing mode play→loading→play ([#114](https://github.com/dascritch/cpu-audio/issues/114)) 
 ...
 * Too many problems in documentations in gh-pages, links to config and chaptering applications are now absolute ([#135](https://github.com/dascritch/cpu-audio/issues/135))
 * Do not show panels when in error state
 * Disambiguation in renaming `findContainer()` in `findCPU()` ([#142](https://github.com/dascritch/cpu-audio/issues/142))
 * Faster instantiation time, removing `<template>` from host page ([#148](https://github.com/dascritch/cpu-audio/issues/148))
 * Documentation re-formated ([#134](https://github.com/dascritch/cpu-audio/issues/134))
 * `HTMLAudioElement.CPU_Update()` and `HTMLAudioElement.CPU_Controller()` going private (vestigial use) ([#143](https://github.com/dascritch/cpu-audio/issues/143))
 ...
 * Using standard ECMA imports ([#110](https://github.com/dascritch/cpu-audio/issues/110))
 * Using ECMA5+ classes, getters, setters, rests & spreads ([#115](https://github.com/dascritch/cpu-audio/issues/115))
 * Renamed files ([#110](https://github.com/dascritch/cpu-audio/issues/110))
 * Better separation between public objects and private methods ([#110](https://github.com/dascritch/cpu-audio/issues/110))
 * Use `npm` to catch dependencies
 * Leaving Google Closure for webpack : too many bugs, too old
 * Using a more aggressive linter
 * Using `css-minify` for compressing css, instead of home-boiled regex ([#56](https://github.com/dascritch/cpu-audio/issues/56))
 * Using `html-minifier` for compressing html, for the exact same reasons ([#56](https://github.com/dascritch/cpu-audio/issues/56))
 * Reduce CSS and HTML compressions operations if no need to freshen ([#56](https://github.com/dascritch/cpu-audio/issues/56))
 * Re-inverted play/pause button semantic ([#116](https://github.com/dascritch/cpu-audio/issues/116))
 ...
 * Playlist are now using the standard API, with special code for CPU-Controller ([#109](https://github.com/dascritch/cpu-audio/issues/109))
 * Some builders are removed from `Element.CPU`, to avoid expose them in public API
 * gh-pages re-organized ([#135](https://github.com/dascritch/cpu-audio/issues/135))
 ...
 * API get inject specific styles features. Very useful for specific annotations presentations. (Preparatory works for [#76](https://github.com/dascritch/cpu-audio/issues/76)) 
 * Adding some examples how to use API ([#66](https://github.com/dascritch/cpu-audio/issues/66), [#101](https://github.com/dascritch/cpu-audio/issues/101))
 * Adding a `convert.durationIso` public method, `datetime=""` attribute in `<time>` needing a specific duration format in ISO 8601
 * User-defined annotation planes can have cue times hidden ([#106](https://github.com/dascritch/cpu-audio/issues/106))
 ...
 * Annotations points are sorted by timecode ([#68](https://github.com/dascritch/cpu-audio/issues/68))
 * Reducing repaints on panels point draws
 * Some events are passive, others are better once-called
 * Won't try to autoplay anymore, as this behaviour is really annoying. You can still revert to this mode with `document.CPU.autoplay` parameter ([#103](https://github.com/dascritch/cpu-audio/issues/103))
 * Returns from `document.CPU.jumpIdAt` and `trigger.hashOrder` aren't used. We can `async` them
 * We try to preload duration metadata when the mouse cursors goes over the timeline on a not know duration and not streamed source ([#88](https://github.com/dascritch/cpu-audio/issues/88))
 ...
 * Primitives for package github ([#86](https://github.com/dascritch/cpu-audio/issues/86)). “to be done”
 ...
 * You can [explicitly indicate the downloadable resource](INSTALL.md#Indicate-a-prefered-downloadable-audio-resource) in case you've got DASH or HLS source ([#12](https://github.com/dascritch/cpu-audio/issues/12) and [#96](https://github.com/dascritch/cpu-audio/issues/96)).
 * Blink to engage to click, optional or to continue an interrupted play ([#93](https://github.com/dascritch/cpu-audio/issues/93))
 * Color and background on `hover:`/`focus:` may be declared, overruled with `--cpu-focus-color` and `--cpu-focus-background` ([#99](https://github.com/dascritch/cpu-audio/issues/99))


Corrections
-----------

 * Action icon is a three-dots instead of a share ([#92](https://github.com/dascritch/cpu-audio/issues/92))
 * Blocked autoplay may do loop on events and stressing reloading audio source ([#93](https://github.com/dascritch/cpu-audio/issues/93))
 * Use css contain to help browser perfs ([#85](https://github.com/dascritch/cpu-audio/issues/85))
 * Correct play from position on the timeline when metadata not preloaded ([#88](https://github.com/dascritch/cpu-audio/issues/88))
 * Stop playing an audiotag when scrobbing another one (mainly for Chrome, ([#89](https://github.com/dascritch/cpu-audio/issues/89)))
 ...
 * Do not intercept keys when modified (as in <kbd>Alt</kbd>+<kbd>←</kbd> [#98](https://github.com/dascritch/cpu-audio/issues/98))
 * Have a proper chapters list : <code>nav > ul > li</code>
 * Some CSS tweaks to help browser perfs ([#85](https://github.com/dascritch/cpu-audio/issues/85))
 * “wait” animation is now more consistent ([#93](https://github.com/dascritch/cpu-audio/issues/93))
 * Avoid inconsequent DOM refreshes ([#28](https://github.com/dascritch/cpu-audio/issues/28))
 ...
 * [Do not show total duration on streaming media](https://github.com/dascritch/cpu-audio/issues/83)
 * [Do not permit time navigation via keys, timeline or handheld navigation on streaming media](https://github.com/dascritch/cpu-audio/issues/83)
 * [Do not store time position in browser on streaming media](https://github.com/dascritch/cpu-audio/issues/83)
 * [Do not recall a position from a stored information in browser on streaming media](https://github.com/dascritch/cpu-audio/issues/83)
 * [No timecode > 0 in links but still 0 to autoplay on streaming media](https://github.com/dascritch/cpu-audio/issues/83)
 * [Forbid direct download on streaming media](https://github.com/dascritch/cpu-audio/issues/83)
 ...
 * [Streamed media gave an confusing duration of `Infinity:NaN:NaN:NaN`](https://github.com/dascritch/cpu-audio/issues/80)
 ...
 * Tests are now compliant on some [browsers' interaction-first policies](https://github.com/dascritch/cpu-audio/issues/17)
 ...
 * [Retravailler un lecteur web audio dans les petites largeurs](https://dascritch.net/post/2019/06/05/Retravailler-un-lecteur-web-audio-dans-les-petites-largeurs) (on issues [#51](https://github.com/dascritch/cpu-audio/issues/51) [#58](https://github.com/dascritch/cpu-audio/issues/58), [#62](https://github.com/dascritch/cpu-audio/issues/62), [#63](https://github.com/dascritch/cpu-audio/issues/63))
 ...
 * [Deux couleurs bizarres en CSS](https://dascritch.net/post/2019/11/13/Deux-couleurs-bizarres-en-CSS) (on issues [#29](https://github.com/dascritch/cpu-audio/issues/29), [#99](https://github.com/dascritch/cpu-audio/issues/99))
 * Tout-terrain WebVTT pour de l'audio (on issues [#57](https://github.com/dascritch/cpu-audio/issues/57), [#59](https://github.com/dascritch/cpu-audio/issues/59))
 ...
 * Waiting line with a more convenient graphic glitch [#61](https://github.com/dascritch/cpu-audio/issues/61)
 ...
 * Add a [waveform in the time-line](https://github.com/dascritch/cpu-audio/issues/6).
 * [Annotations API](https://github.com/dascritch/cpu-audio/issues/57).
 * A handle on the time-line is appearing when hovering with a pointer.
 * On small screens, time-line now goes out of between-buttons space, using the full WC width to [gain precision on small screens](https://github.com/dascritch/cpu-audio/issues/58).
 ...
 * Can avoid to [not force preloading metadata](https://github.com/dascritch/cpu-audio/issues/70) and indicates an estimation instead.
 * Panels (chapters and playlist) have a hide-able title.
 * [Translating specific VTT markup into HTML tags](https://github.com/dascritch/cpu-audio/issues/59)
 ...
 * Less visible [graphic glitch](https://github.com/dascritch/cpu-audio/issues/61) on the loading line.
 * Error mode style correction.
 * Chapters now use [annotations API](https://github.com/dascritch/cpu-audio/issues/57).
 * Texts in chapters panel should use more space.
 * Texts in chapters panel [should not accept embeded links](https://github.com/dascritch/cpu-audio/issues/59)
 * Boxed playing (with a end timecode) now uses [annotations API](https://github.com/dascritch/cpu-audio/issues/57).
 ...
 * Chapters live editor don't use anymore `<input type="time">` as [it is not a duration input, nor correctly working in 12-hours locale](https://github.com/dascritch/cpu-audio/issues/63) à la Américaine Way
